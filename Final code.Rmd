...
## Removal of columns without useful information
...
```{r Drugs and subtypes relationship}
ae_d7 <-data.frame(matrix(nrow = nrow(effective_d7),ncol = length(effective_in_all_doses)),row.names = rownames(brain_cancer))
...
for(i in c(1:length(effective_in_all_doses))){zv <- rep(i,4);res <- c(res, zv)}
...
# Compare drugs and achilles targets

```{r Drugs vs Achilles targets}
