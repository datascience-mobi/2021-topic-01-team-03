...
to_which_dose_=list(apply(extra_drugs, 2, which.min))
extra_drugs_<-(rep(to_which_dose_, 8))
a<-function(x){
  for(i in 1:8)
  which(x==i)
}
k<-lapply(to_which_dose_, a)
#find another way
...
brain_cancer_exp_t[,-bc_exp_removed]
...
bc_exp_shap = apply(brain_cancer_exp_t,2,shapiro.test)
bc_exp_shap_res = sapply(brain_cancer_exp_t,`[`,  c("p.value")) # genes which do have a normal distributed expression; which threshold?
bc_exp_norm = which(bc_cnv_shap_res > 0.95)
