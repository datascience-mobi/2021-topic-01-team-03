...
min_eff_test=min(apply(ae_d7,2,min))
max_eff=max(apply(ae_d7,2,max))

# Creating a sequence for the interval

interval=seq(from=min_eff, to=max_eff, by=((abs(min_eff)+max_eff))/10)

# Cutting a vector into these intervals

x=effective_in_all_doses_df[,1]
tryout=cut(x, breaks=interval, right=TRUE, include.lowest = TRUE)
...
#to=unlist(lapply(eff_intervals,max))
