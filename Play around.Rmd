...
View(apply(compare_subtypes, 2, plot))
```

```{r}
ae_d4 <-data.frame(matrix(nrow = 34,ncol = length(effective_in_all_doses)),row.names = rownames(brain_cancer))
colnames(ae_d4)=effective_in_all_doses

for (i in 1:length(effective_in_all_doses)){
  ae_d4[,i]=effective_d4[,grep(effective_in_all_doses[i], colnames(effective_d4))]
}

glioma_ae_d4=ae_d4[glioma,]

glioblastoma_ae_d4=ae_d4[glioblastoma,]

astrocytoma_ae_d4=ae_d4[astrocytoma,]

medulloblastoma_ae_d4=ae_d4[medulloblastoma,]

compare_subtypes2<-data.frame(matrix(nrow = 4,ncol = length(effective_in_all_doses)),row.names = c("glioma", "glioblastoma", "astrocytoma", "medulloblastoma"))
colnames(compare_subtypes2)=effective_in_all_doses

compare_subtypes2[1,]<-apply(glioma_ae_d4, 2, mean)
compare_subtypes2[2,]<-apply(glioblastoma_ae_d4, 2, mean)
compare_subtypes2[3,]<-apply(astrocytoma_ae_d4, 2, mean)
compare_subtypes2[4,]<-apply(medulloblastoma_ae_d4, 2, mean)

apply(compare_subtypes2, 2, plot)
```

##Looking at gene expression in brain cancer cells

```{r}
# summary of brain_cancer_exp
...
# prism histogram
hist(mat.prism.exp, breaks = 100, xlim = range(0,10), ylim = range(0,200000), xlab = "prism.exp")
abline(v = c(sum.prism.exp[2:5]), col = "red", lwd = 1, lty = 1)

#Observation: 25% of genes are KO, around 50% have an expression over 2,5 TPM. 

#Which TPM is considered to be an overexpression?

#Which genes are completely KO?
KO_genes=colnames(brain_cancer_exp[,as.numeric(which(apply(brain_cancer_exp, 2, mean)==0))])
```

```{r}
# brain_cancer_achilles data frame as vector
mat.brain_cancer_achilles <- as.matrix(brain_cancer_achilles)
vec.brain_cancer_achilles <- as.vector(mat.brain_cancer_achilles)
sum.brain_cancer_achilles <- summary(vec.brain_cancer_achilles)
dim.brain_cancer_achilles <- dim(brain_cancer_achilles)
datp.brain_cancer_achilles <- dim.brain_cancer_achilles[1] * dim.brain_cancer_achilles[2]

# prism histogram
hist(mat.brain_cancer_achilles, breaks = 200, xlim = range(-2.5,1), xlab = "brain_cancer_achilles")
abline(v = c(sum.brain_cancer_achilles[2:5]), col = "red", lwd = 1, lty = 1)

```

```{r}
# summary of brain_cancer_cnv

mat.prism.cnv <- as.matrix(brain_cancer_cnv)
