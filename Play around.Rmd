...
library(effsize)
cohen_d_values <- vector(mode = "list",length = ncol(brain_cancer_exp))
for(i in 1:ncol(brain_cancer_exp)) {         
  cohen_d_values[[i]] <- cohen.d(non_bc_exp[,i], brain_cancer_exp[,i])
}
```
...
library(gtools)
bc_exp_mean=apply(brain_cancer_exp,2, mean)
non_bc_exp_mean=apply(non_bc_exp,2,mean)
foldchange(bc_exp_mean,non_bc_exp_mean)
cohen.d(bc_exp_mean,non_bc_exp_mean)
```
