...
##Regression model to select relevant genes
```{r}
...
for(i in 1:ncol(effective_in_all_doses_df)) {         
  ks_drugs[[i]] <- ks.test(as.numeric(effective_in_all_doses_df[,i]),"punif", mean(effective_in_all_doses_df[,i]), sd(effective_in_all_doses_df[,i]), alternative = "two.sided")
}
...
ks_pvalue_min=min(ks_drugs_pvalue)

test_vector=c(effective_in_all_doses_df[,51])
test_ks=ks.test(as.numeric(test_vector),"punif",mean(test_vector), sd(test_vector), alternative="two.sided")



                            
the_genes=brain_cancer_exp[names,names(volcano_filtered_genes)]
View(ae_d7)
View(the_genes)

d_g<-data.frame(matrix(nrow = 35,ncol = 51))
rownames(d_g)=c("Drugs",names)
d_g[1,]<-apply(ae_d7,2,mean)

set.seed(333)
for(i in 1:34){
  d_g[i+1,]<-sample(the_genes[i,],51)
}
 d_g<-as.data.frame(t(d_g))

#summary(lm(Drugs~a+b+c,d_g))


#loop intent
aa<-lm(Drugs~Genes,data = a)

r_squared<- vector(length = 50000)

d_g<-data.frame(matrix(nrow = 51,ncol = 2))
colnames(d_g)=c("Drugs","Genes")
d_g[,1]<-apply(ae_d7,2,mean)
d_g[,2]<-sample(the_genes[1,],51)

for(i in 1:50000) {
  r_squared[i] <- summary(lm())
}
```
