...
length(setdiff(colnames(doses_subsets_2[[6]]),colnames(brain_cancer[,d6_extra])))
...
sapply(doses_for_list_2,check_for_differences)
...
length(setdiff(colnames(all_effective_drugs[[1]]),colnames(all_effective_drugs_2[[1]])))
...
non_bc_exp=as.data.frame(scale(as.matrix(non_bc_exp)))
brain_cancer_exp_w=as.data.frame(scale(as.matrix(brain_cancer_exp)))

hist(as.numeric(brain_cancer_exp_w[10,]),breaks=50,xlim = range(-3,3))

# Performance of Wilcoxon
wilcox.test(as.numeric(non_bc_exp[,24]),as.numeric(brain_cancer_exp_w[,24]),alternative = "two.sided", paired = F)
...
```{r}
bc_exp_mean=apply(brain_cancer_exp[,b_correction],2, mean)
non_bc_exp_mean=apply(non_bc_exp[,b_correction],2,mean)

logfc<-log2(bc_exp_mean/non_bc_exp_mean)
logpv=-log10(b_correction)
df <- data.frame(logfc,logpv)
library(ggplot2)
ggplot(df, aes(x=logfc, y=logpv)) +
  geom_point()

volcano_filtered_genes=logfc[which(logfc>=1|logfc<=-1)]
View(volcano_filtered_genes)
```
