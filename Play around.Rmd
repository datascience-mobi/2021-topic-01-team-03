...
```{r}
brain_canancer_treat=prism.treat[colnames(brain_cancer),] #selecting only the drugs ve previously filtered out from brain_cancer

dt1=brain_canancer_treat[colnames(d1),]
dt2=brain_canancer_treat[colnames(d2),]
dt3=brain_canancer_treat[colnames(d3),]
dt4=brain_canancer_treat[colnames(d4),]
dt5=brain_canancer_treat[colnames(d5),]
dt6=brain_canancer_treat[colnames(d6),]
dt7=brain_canancer_treat[colnames(d7),]
dt8=brain_canancer_treat[colnames(d8),]

#not sure this sorting makes sense now for the brain_cancer_treat, but now we have it in case we need it in the future

summary(brain_canancer_treat)#can be useful to identify trends or come up with some new ideas for filtering, interesting columns can be looked at separetely, for example:
summary(brain_canancer_treat$disease.area)
...
```
>>>>>>> b52712a29a6bfdaca9a50177d68eef04d4c7ab3e
...
## Silhouette plot to check the number of clusters
```{r}
sil_plot = silhouette(coordinates)

## das habe ich nicht hingekriegt mit dem Silhouette plot
```
```{r}
