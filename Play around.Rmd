...
wilcox.test(as.numeric(non_bc_exp[,1]),as.numeric(brain_cancer_exp[,1]),paired = F)
...
wilcox_genes <- vector(mode = "list",length = 19177)
for(i in 1:19177) {         
  wilcox_genes[[i]] <- wilcox.test(as.numeric(non_bc_exp[,i]),as.numeric(brain_cancer_exp[,i]),paired = F)
}
...
wilcox_pvalue_genes <- vector(length = 19177)
for(i in 1:19177) {
  wilcox_pvalue_genes[i] <- wilcox_genes[[i]]$p.value
}
