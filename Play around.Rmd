...
#just a quick check how many doses there are in total: must be all dosages after NA removal
...
```
##Select the drugs that are effective in all doses
...
```{r}
new_rownames<-function(x){
  a<-gsub(pattern = "::.*", replacement = "", rownames(x))
  return(a)
}
rownames(effective_t1)<-new_rownames(effective_t1)#has two drugs that repeat themselves

duplicated(new_rownames(effective_t3))
...
rownames(effective_t2)<-new_rownames(effective_t2)
rownames(effective_t3)<-new_rownames(effective_t3)
rownames(effective_t4)<-new_rownames(effective_t4)
rownames(effective_t5)<-new_rownames(effective_t5)
rownames(effective_t6)<-new_rownames(effective_t6)
rownames(effective_t7)<-new_rownames(effective_t7)
rownames(effective_t8)<-new_rownames(effective_t8)





treat_4_doses=brain_cancer_treat[effective_in_4_doses,]
treat_6_doses=brain_cancer_treat[effective_in_6_doses,]
treat_all_doses=brain_cancer_treat[effective_in_all_doses,]
```
