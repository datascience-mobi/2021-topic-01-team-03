...
wilcox.test(as.numeric(non_bc_exp[,234]),as.numeric(brain_cancer_exp[,234]),alternative = "two.sided", paired = F)

wilcox_genes <- vector(mode = "list",length = 100)
for(i in 1:100) {         
...
summary(lm(Drugs~colnames()),data=d_g)

#try onj a single cell line
d_g<-data.frame(matrix(nrow = 2,ncol = 51))
rownames(d_g)=c("Drugs","Genes")
d_g[1,]<-apply(ae_d7,2,mean)
d_g[2,]<-sample(the_genes[1,],51)
 d_g<-as.data.frame(t(d_g))
summary(lm(Drugs~Genes,data = d_g))
