...
# Listing subsets of dosages to make them easier to use 
doses_subsets=list(d1,d2,d3,d4,d5,d6,d7)
...
General consideration: Characteristic genes are genes that have a small variance in a subtype which is not present in another subtype. 

```{r}
# Finding candidate genes by hypothesis testing

## checking data for normality by applying a shapiro test on each column 

bc_cnv_shap = apply(brain_cancer_cnv,2,shapiro.test)
bc_cnv_shap_res = sapply(bc_cnv_shap,`[`,  c("p.value"))
bc_cnv_norm = which(bc_cnv_shap_res > 0.95) # genes which do have a normally distributed expression -> these are not interesting for us 

# Should be removed; than Wilcoxon test
