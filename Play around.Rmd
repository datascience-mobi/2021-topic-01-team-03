...
#q<-apply(brain_cancer_exp,2,var)
#w<-which(q==0)
#View(brain_cancer_exp[,-w])
#s<-brain_cancer_exp[,-w]
...
# Selecting genes with a p value smaller than 0.5%
bc_exp_norm = which(exp_pvalue > 0.95)
bc_exp_not_norm=brain_cancer_exp[,-bc_exp_norm]
...
##Performance of Wilcoxon test to extract genes which are differentially expressed.
```{r eval=FALSE}
# Removal of these columns
...
non_bc_exp=non_bc_exp[,-c(bc_exp_norm)]
# Taking the mean/ median of all non brain cancer cell lines as reference 
non_bc_exp_mean_t=colMeans(non_bc_exp)
non_bc_exp_mean=as.data.frame(t(non_bc_exp_mean_t))
View(non_bc_exp_mean)

# Wilcoxon test
wilcox_cells <- vector(mode = "list",length = 34)
for(i in 1:34) {         
  wilcox_cells[[i]] <- wilcox.test(as.numeric(non_bc_exp_mean[1,]),as.numeric(bc_exp_not_norm[i,]))
}

wilcox_pvalue <- vector(length = 34)
for(i in 1:34) {         
  wilcox_pvalue[i] <- wilcox_cells[[i]]$p.value
}

length(which(wilcox_pvalue<=0.05))
#24 brain cancer cell lines are different in gene expression comparing to other tumor types
names[-which(wilcox_pvalue<=0.05)] #names of cell lines which are similar in expression to the rest of tumor

brain_cancer_cl[-which(wilcox_pvalue<=0.05),"disease_subtype"]
#here we can see which types of cells are similar in expression to other tumors
```
...
# Removal of lines with NA from new dataframe
non_bc_exp=non_bc_exp[rowSums(is.na(non_bc_exp)) == 0,]
#non_bc_exp=non_bc_exp[,-c(bc_exp_norm)] #deactivation of this line since we don't know whether Shapiro was necessary

# Performance of Wilcoxon
#wilcox.test(as.numeric(non_bc_exp[,1]),as.numeric(bc_exp_not_norm[,1]),paired = F) #same procedure
wilcox.test(as.numeric(non_bc_exp[,1]),as.numeric(brain_cancer_exp[,1]),alternative = "two.sided", paired = F)

#wilcox_genes <- vector(mode = "list",length = 18699)
#for(i in 1:18699) {         
#  wilcox_genes[[i]] <- wilcox.test(as.numeric(non_bc_exp[,i]),as.numeric(bc_exp_not_norm[,i]),paired = F)
#}

wilcox_genes <- vector(mode = "list",length = ncol(brain_cancer_exp))
for(i in 1:ncol(brain_cancer_exp)) {         
  wilcox_genes[[i]] <- wilcox.test(as.numeric(non_bc_exp[,i]),as.numeric(brain_cancer_exp[,i]), alternative = "two.sided", paired = F)
}

#wilcox_pvalue_genes <- vector(length = 18699)
#for(i in 1:18699) {         
#  wilcox_pvalue_genes[i] <- wilcox_genes[[i]]$p.value
#}

wilcox_pvalue_genes <- vector(length = ncol(brain_cancer_exp))
for(i in 1:ncol(brain_cancer_exp)) {
  wilcox_pvalue_genes[i] <- wilcox_genes[[i]]$p.value
}
...
#H0: expression value for the gene comes from the same distribution
#H1: Group gene expression values vome from two distributions with different central measures
