...
brain_cancer_exp_t = as.data.frame(t(as.matrix(brain_cancer_exp)))

# Filtering *rows* using the dplyr::filter function as Stefan suggested
#brain_cancer_exp_t %>%
#filter(x==min(x))
#bc_exp_removed = apply(brain_cancer_exp, 2, function(x) min(x) == max(x))


# Transposing dataframe again
View(brain_cancer_exp)
...
exp_list <- vector(mode = "list", length = ncol(s))
for(i in 1:ncol(s)) {         
  exp_list[[i]] <- s[ , i]
}

#brain_cancer_exp_t = as.data.frame(t(as.matrix(brain_cancer_exp_t)))

# checking data for normality by applying a shapiro test on each column 

bc_exp_shap = lapply(exp_list,shapiro.test)

exp_pvalue <- vector(length = ncol(s))
for(i in 1:ncol(s)) {         
  exp_pvalue[i] <- bc_exp_shap[[i]]$p.value
} # genes which do have a normal distributed expression; which threshold?
View(bc_exp_shap)
...
```{r}
wilcox.test(as.numeric(non_bc_exp[,1]),as.numeric(bc_exp_not_norm[,1]),paired = F)
