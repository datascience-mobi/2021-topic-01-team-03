...
## cnv
res <- brain_cancer_cnv[,which(colnames(brain_cancer_cnv) %in% training_targets.solo )]

training_cnv <- apply(res,2,function(x){mean(x, na.rm = TRUE)})

training_cnv <- as.data.frame(training_cnv)

training_cnv[,2] <- row.names(training_cnv)

training_cnv <- training_cnv[order(training_cnv[,2]),]
...
for(i in training_targets.solo){
 training_prism <- cbind(training_prism,target.to.prism(i))
}
...
# merge
training <- cbind(training, training_exp, training_cnv, t(training_prism))

training <- training[,c(3,1,2,4,6)]

colnames(training) <- c("targets","achilles","exp","cnv","prism")
...
## Regression model
```{r}
pairs(~prism+achilles+exp+cnv, data = training, )

l1 <- lm(prism~achilles+exp+cnv, data = training)
summary(l1)
```
...
l2 <- lm(prism~achilles+exp+cnv, data = dat.train)
summary(l2)
...
multiple.lm.values <- matrix(ncol = 8)
...
res <- c(summary(l2)$fstatistic, summary(l2)$adj.r.squared, l2$coefficients)

multiple.lm.values <- rbind(multiple.lm.values,res) 

}
hist(multiple.lm.values[,4], breaks = 50)
hist(multiple.lm.values[,5], breaks = 50)
hist(multiple.lm.values[,6], breaks = 50)
hist(multiple.lm.values[,7], breaks = 50)
hist(multiple.lm.values[,8], breaks = 50)
summary(multiple.lm.values[,4])
```
...
line_target.search <- function(cellline, target){
...
#target <- "LPL"
  ...
  for(i in 1:nrow(prism.treat)){
    ...
    target.in.line <- unlist(strsplit(as.character(prism.treat[i,7]), split = ", ")) == target
    
    if(sum(target.in.line, na.rm = TRUE) > 0){
      drug.line <- cbind(drug.line, i)
    }
  }
  
  drugs.from.target <- unique(prism.treat[drug.line,1])
  ...
  for(i in 1:ncol(prism)){
    ...
    drug.in.col <- unlist(strsplit(as.character(colnames(prism)[i]), split = "::"))[1] == drugs.from.target
    
    if(sum(drug.in.col, na.rm = TRUE) > 0){
      drug.col <- cbind(drug.col, i)
    }
  }
  ...
  for(i in drug.col){
     ...
     if( 6.25 < as.numeric(unlist(strsplit(as.character(colnames(prism)[i]), split = "::"))[2]) && as.numeric(unlist(strsplit(as.character(colnames(prism)[i]), split = "::"))[2]) <= 10 )
       {drug.dose <- cbind(drug.dose,i) } # right dose for d7???
    
  }
  
  # prism value
  
  lts.prism <- mean(as.numeric(prism[grep(cellline,row.names(prism)), drug.dose]), na.rm = TRUE)
...
training1 <- data.frame()

for (i in celllines[,2]){
  print(i)
  for (j in training_targets.solo){
    #line_target.search(i, j)
    print(j)
    training1 <- rbind(training1, line_target.search(i,j))
  }
}
```
...
Improved regression model
```{r}
training1 <- na.omit(training1) # delete 66 NA rows

pairs(~prism+IQANK1+RAB11FIP1+FOXG1+LPIN3+ERVMER34.1+PTN+VRK2+achilles+exp+cnv, data = training1, pch = 20, cex = 0.5)

l3 <- lm(prism~IQANK1+RAB11FIP1+FOXG1+LPIN3+ERVMER34.1+PTN+VRK2+achilles+exp+cnv+prism, data = training1)
summary(l3)

pca3 <- prcomp(training1[,-c(11)])
summary(pca3)
for(i in 1:(ncol(training1)-1)){
par(las=2);barplot(pca3$rotation[,i],horiz = TRUE, main = paste("PC",i))

}

l.pca3 <- lm(training1$prism~pca3$x)
summary(l.pca3)
```
...
initial_regression_1<-summary(lm(training1$prism~pca3.leftx)) # 3.(model B)
...
search.prism <- function(search.mode = "t", cellline, target.or.drug, dose = 8){
  ...
  #dose tabel: d1-d8
  doses.table <- data.frame()
  doses.table <- c(0,0.00061034,0.0024414,0.00976562,0.0390625,0.15625,0.625,2.5,10)
  bot.v <- vector()
  for(i in 1:c(length(doses.table)-1)){bot.v <- cbind(bot.v,c(doses.table[i]+(doses.table[c(i+1)]-doses.table[i])/2)) }
  bot.v <- cbind(bot.v, 10)
  doses.table <- rbind(doses.table,bot.v)
  doses.table[2,1] <- 0
  row.names(doses.table) <- c("Original dosis","End of intervall")
  colnames(doses.table) <- c("d0","d1","d2","d3","d4","d5","d6","d7","d8")
  ...
  # dosis intervall
  lower.v <- doses.table[2,dose]
  upper.v <- doses.table[2,dose+1]
...
training3 <- data.frame()

for (i in celllines[,2]){
  print(i)
  for (j in effective_in_all_doses){
    #line_target.search(i, j)
    print(j)
    training3 <- rbind(training3, search.prism("d",i,j,8))
  }
}
```
...
for (i in celllines[,2]){
  ...
  for (j in training_targets.solo){
    #line_target.search(i, j)
    print(j)
    training2 <- rbind(training2, search.prism("t",i,j,8))
  }
}
```
