...
# doses 7 is nearly the same number (by 5 higher in medulloblastoma)
...
# testing the number of clusters by elbow-plot

wss <- sapply(1:8, function(k){
  kmeans(x=t(gb7), centers = k)$tot.withinss
})
plot(1:8,wss,type = 'b',pch = 19, main = "Glioblastoma", xlab = "Number of clusters K",
     ylab = "Total within-clusters sum of squares")

wss <- sapply(1:8, function(k){
  kmeans(x=t(glioma_ae_d7), centers = k)$tot.withinss
})
plot(1:8,wss,type = 'b',pch = 19, main = "Glioma", xlab = "Number of clusters K",
     ylab = "Total within-clusters sum of squares")

# I want to visualize the binary question drug response against no drug response. For that reason I have to take the normal d7 datasets without filtering for the effective drugs 
...
# and for the non-effective drugs 

not_effective_drugs <- function(x){
  not_effective_x<- x[,which(apply(x,2,mean)>0.2)]
  return(not_effective_x)
}

not_effective_gb7=not_effective_drugs(gb7)
not_effective_mb7=not_effective_drugs(mb7)
not_effective_g7=not_effective_drugs(g7)
not_effective_ac7=not_effective_drugs(ac7)

# I am not sure, if i need the means of the effective or non-effective drugs 

effective_gb7_mean = colMeans(effective_gb7)
#effective_mb7_mean = colMeans(effective_mb7)
#effective_g7_mean = colMeans(effective_g7)
#effective_ac7_mean = colMeans(effective_ac7)

not_effective_gb7_mean = colMeans(not_effective_gb7)
#not_effective_mb7_mean = colMeans(not_effective_mb7)
#not_effective_g7_mean = colMeans(not_effective_g7)
#not_effective_ac7_mean = colMeans(not_effective_ac7)

#gb7_mean = colMeans(gb7)

# Performing k-means clustering

set.seed(123)
gb_cluster <-  kmeans(x=t(gb7), centers = 2, nstart =10)
gb_cluster_df = data.frame(Cluster = gb_cluster$cluster)

#gb_cluster$cluster

# Visualization via ggplots

library(ggplot2)
#ggplot(data = gb_cluster_df) +
  #geom_point(aes(x=, y=)
...
## Compute k-means with k = x 
x <- 6 

km_astrocytoma_ae_d7 = kmeans(t(astrocytoma_ae_d7), centers = x) 

y <- 4 ## cut off height was choosen as 8 from previous dendogramm 

clust_km <- km_astrocytoma_ae_d7$cluster 

# Create a new data frame storing these results
# km_astrocytoma_ae_d7_complete <- mutate(as.data.frame(t(astrocytoma_ae_d7)), cluster = clust_km)

plot(km_astrocytoma_ae_d7_complete)

# Plot the positions of the players and color them using their cluster for height = 20
# ggplot(astrocytoma_ae_d7_mean, aes(x = x), color = factor) +
 # geom_scatter()
...
km_astrocytoma_d7 <- kmeans(astrocytoma_ae_d7, 1, nstart = 25)
print(km_astrocytoma_d7)

```


```{r}
clusters_x <- cutree(clust_astro_ae_d7, k=x) ## 4 clusters could be ideal 
#astro_ae_d7_complete <- mutate(astrocytoma_ae_d7, cluster = clusters_x)

ggplot(astro_ae_d7_complete, aes(x = x, y = y, color = factor(cluster))) +
  geom_point()
```
...
```{r}
value <- colMeans(astrocytoma_ae_d7)
hist(value, breaks = 500)

boxplot(astrocytoma_ae_d7, xlba="Efficiency", ylab="Drug")
boxplot(glioma_ae_d7, xlab="Efficiency", ylab="Drug")
boxplot(glioblastoma_ae_d7, xlba="Efficiency", ylab="Drug")
boxplot(medulloblastoma_ae_d7, xlba="Efficiency", ylab="Drug")
...
```{r}
par(mfrow=c(2,2))
ggplot(means_eff, aes(x = Drugs , y = Astrocytoma)) + 
        geom_point()

ggplot(means_eff, aes(x = Drugs , y = Glioma)) + 
          geom_point()
         
ggplot(means_eff, aes(x = Drugs , y = Glioblastoma)) +   
         geom_point()

ggplot(means_eff, aes(x = Drugs , y = Medulloblastoma)) +   
         geom_point()



new_means_eff <- data.frame(x = means_eff$Drugs, 
                            y = c(means_eff$Astrocytoma, means_eff$Glioma, means_eff$Glioblastoma, means_eff$Medulloblastoma), 
                            subtype = c(rep("Astrocytoma", nrow(means_eff)),
                                      rep("Glioma", nrow(means_eff)),
                                      rep("Glioblastoma", nrow(means_eff)),
                                      rep("Medulloblastoma", nrow(means_eff))))
ggplot(new_means_eff, aes(x = x, y = y, color = subtype)) + 
...
```{r}
new_means_eff <- data.frame(x = means_eff$Drugs, 
                            y = c(means_eff$Glioma, means_eff$Medulloblastoma), 
                            subtype = c(
                                      rep("Glioma", nrow(means_eff)),
                                      rep("Medulloblastoma", nrow(means_eff))))
ggplot(new_means_eff, aes(x = x, y = y, color = subtype)) + 
...
## in 38 von 51 Fällen sind die Drogen auf die Medulloblastoma wirksamer als auf die Glioma  

new_means_eff <- data.frame(x = means_eff$Drugs, 
                            y = c(means_eff$Glioblastoma, means_eff$Medulloblastoma), 
                            subtype = c(
                                      rep("Glioblastoma", nrow(means_eff)),
                                      rep("Medulloblastoma", nrow(means_eff))))
ggplot(new_means_eff, aes(x = x, y = y, color = subtype)) + 
...
## in 37 von 51 Fällen sind die Drogen auf die Medulloblastoma wirksamer als auf die Glioblastoma 


new_means_eff <- data.frame(x = means_eff$Drugs, 
                            y = c(means_eff$Astrocytoma, means_eff$Medulloblastoma), 
                            subtype = c(
                                      rep("Astrocytoma", nrow(means_eff)),
                                      rep("Medulloblastoma", nrow(means_eff))))
ggplot(new_means_eff, aes(x = x, y = y, color = subtype)) + 
                         ggtitle("Means of subtypes") +
                          xlab("Drugs") + 
...
## in 38 von 51 Fällen sind die Drogen auf die Medulloblastoma wirksamer als auf die Astrocytoma  
```
...
# Create a new data frame storing these results
km_astrocytoma_ae_d7_complete <- as.data.frame(t(astrocytoma_ae_d7), cluster = clust_km)
...
k.max <- 15
wss <- sapply(1:k.max, 
              function(k){kmeans(astrocytoma_ae_d7, 2, nstart=50,iter.max = 15 )$tot.withinss})
wss
plot(1:k.max, wss,
