...
```
## kmeans clustering of the meansof all subtypes  
```{r}
km_df <- data.frame(Astrocytoma = astrocytoma_ae_d7_mean,
                        Glioma = glioma_ae_d7_mean, 
                        Glioblastoma = glioblastoma_ae_d7_mean,  
                        Medulloblastoma = medulloblastoma_ae_d7_mean)
km <- kmeans(km_df, 4, iter.max = 10, nstart = 1)
km
```
## Elbowplot for the clusters 

```{r}
# testing the number of clusters by elbow-plot

wss <- sapply(1:8, function(k){
  kmeans(x=km_df, centers = k)$tot.withinss
})
